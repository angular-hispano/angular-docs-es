...
Si alguno de los archivos cacheados cambia, el hash del archivo cambia en `ngsw.json`. Este cambio hace que el service worker de Angular trate el conjunto activo de archivos como una versión nueva.

ÚTIL: El proceso de compilación crea el archivo de manifiesto `ngsw.json` usando la información de `ngsw-config.json`.

Con el comportamiento de versionado del service worker de Angular, un servidor puede garantizar que la aplicación Angular siempre tenga un conjunto coherente de archivos.

#### Verificaciones de actualización

Cada vez que el usuario abre o actualiza la aplicación, el service worker de Angular verifica si hay actualizaciones revisando si el manifiesto `ngsw.json` cambió.
...
Uno de los posibles efectos secundarios del almacenamiento en caché prolongado es conservar accidentalmente un recurso que no es válido.
...
Un service worker ignora esas restricciones y, en la práctica, almacena en caché toda la aplicación por un periodo prolongado.
...
Si es necesario, el service worker entra en un modo seguro en el que las solicitudes regresan a la red. El service worker no usa su caché si existe un alto riesgo de servir contenido roto, desactualizado o no válido.
...
* Errores durante el proceso de compilación podrían generar recursos actualizados sin que `ngsw.json` se actualice (también puede suceder lo inverso: que `ngsw.json` se actualice sin que cambien los recursos)

#### Contenido sin hash

Los únicos recursos que tienen hashes en el manifiesto `ngsw.json` son los que estaban presentes en el directorio `dist` cuando se generó el manifiesto.
Otros recursos, especialmente los que se cargan desde CDNs, tienen contenido que es desconocido en tiempo de compilación o se actualiza con más frecuencia de lo que se despliega la aplicación.

Si el service worker de Angular no tiene un hash para verificar que un recurso sea válido, igualmente almacena su contenido en caché. Al mismo tiempo, respeta los encabezados de caché HTTP usando una política de *stale while revalidate*.
El service worker de Angular continúa sirviendo un recurso incluso después de que sus encabezados de caché HTTP indiquen que ya no es válido. Al mismo tiempo, intenta actualizar el recurso vencido en segundo plano.
Así, los recursos sin hash que estén rotos no permanecen en la caché más allá de su vida útil configurada.

### Pestañas de la aplicación

Puede ser problemático para una aplicación si la versión de los recursos que recibe cambia de repente o sin aviso.
Consulta la sección [Versiones de la aplicación](#application-versions) para ver la descripción de este tipo de problemas.
...
IMPORTANTE: Esta garantía es **más fuerte** que la que ofrece el modelo de distribución web estándar. Sin un service worker, no hay garantía de que el código cargado de forma diferida pertenezca a la misma versión que el código inicial de la aplicación.
...
En algunos casos, podrías querer omitir el service worker por completo y permitir que el navegador maneje la solicitud.
Un ejemplo es cuando dependes de una característica que actualmente no está soportada en los service workers, como [informar el progreso en cargas de archivos](https://github.com/w3c/ServiceWorker/issues/1141).

Para omitir el service worker, establece `ngsw-bypass` como encabezado de la solicitud o como parámetro de consulta.
...
El service worker procesa todas las solicitudes a menos que se [omita explícitamente](#bypassing-the-service-worker).
...
Por ejemplo, si el service worker no recibe respuesta, crea un estado [504 Gateway Timeout](https://developer.mozilla.org/docs/Web/HTTP/Status/504) para devolverlo. El estado `504` en este ejemplo puede aparecer porque el servidor está sin conexión o el cliente se desconectó.
...
`NORMAL` indica que el service worker está operando con normalidad y no se encuentra en un estado degradado.
...
| `SAFE_MODE`                 | El service worker no puede garantizar la seguridad de usar los datos cacheados. Ocurrió un error inesperado o todas las versiones cacheadas no son válidas. Todo el tráfico se servirá desde la red, ejecutando la menor cantidad posible de código del service worker. |

En ambos casos, la anotación entre paréntesis proporciona el error que provocó que el service worker entrara en el estado degradado.
...
El navegador a veces detiene un service worker inactivo para conservar memoria y capacidad de procesamiento, y crea uno nuevo en respuesta a eventos de red.
...
* Cuando usas las herramientas para desarrolladores, el service worker se mantiene ejecutándose en segundo plano y nunca se reinicia.
    Esto puede provocar que el comportamiento con DevTools abierto difiera del que experimentaría una persona usuaria.

* Si miras el visor de Cache Storage, la caché con frecuencia está desactualizada.
...
Si configuras una redirección desde la ubicación anterior, como `example.com`, hacia la nueva ubicación `www.example.com`, el worker deja de funcionar.
...
Para solucionar esto, quizá tengas que desactivar el worker antiguo usando una de las técnicas anteriores: [Mecanismo a prueba de fallos](#fail-safe) o [Safety worker](#safety-worker).
