...
## ¿Qué es la hidratación? {#what-is-hydration}
...
## ¿Por qué es importante la hidratación? {#why-is-hydration-important}
...
## ¿Cómo se habilita la hidratación en Angular? {#how-do-you-enable-hydration-in-angular}
...
### Usando Angular CLI {#using-angular-cli}
...
## Captura y reproducción de eventos {#capturing-and-replaying-events}
...
### Cómo funciona la reproducción de eventos {#how-event-replay-works}
...
### Manipulación Directa del DOM {#direct-dom-manipulation}
...
Lo mejor es refactorizar tu componente para evitar este tipo de manipulación del DOM. Intenta usar las APIs de Angular para realizar este trabajo, si puedes. Si no puedes refactorizar este comportamiento, usa el atributo `ngSkipHydration` ([descrito abajo](#how-to-skip-hydration-for-particular-components)) hasta que puedas refactorizar hacia una solución compatible con la hidratación.

### Estructura HTML válida {#valid-html-structure}
...
### Configuración de Preservación de Espacios en Blanco {#preserve-whitespaces-configuration}
...
### Zone.js personalizado o Noop aún no son compatibles {#custom-or-noop-zonejs-are-not-yet-supported}
...
## Cómo omitir la hidratación para componentes específicos {#how-to-skip-hydration-for-particular-components}

Algunos componentes pueden no funcionar correctamente con la hidratación habilitada debido a algunos de los problemas mencionados anteriormente, como la [Manipulación Directa del DOM](#direct-dom-manipulation). Como solución alternativa, puedes agregar el atributo `ngSkipHydration` a la etiqueta de un componente para omitir la hidratación del componente completo.
...
## Tiempo de Hidratación y Estabilidad de la Aplicación {#hydration-timing-and-application-stability}
...
### Depurar la estabilidad de la aplicación {#debugging-application-stability}

La utilidad `provideStabilityDebugging` ayuda a identificar por qué tu aplicación no logra estabilizarse. Esta utilidad se proporciona por defecto en modo dev cuando se usa `provideClientHydration`. También puedes agregarla manualmente a los providers de la aplicación para usar en bundles de producción o cuando se usa SSR sin hidratación. La función registra información en la consola si la aplicación tarda más de lo esperado en estabilizarse.

```typescript
import {provideStabilityDebugging} from '@angular/core';
import {bootstrapApplication} from '@angular/platform-browser';
import 'zone.js/plugins/task-tracking'; // Usa esto si tienes Zone.js con `provideZoneChangeDetection`
...
Cuando está habilitada, la utilidad registra las tareas pendientes (`PendingTasks`) en la consola. Si tu aplicación usa Zone.js, también puedes importar `zone.js/plugins/task-tracking` para ver qué macrotasks están impidiendo que la Angular Zone se estabilice. Este plugin proporciona el stack trace de la creación de la macrotask, ayudándote a identificar eficazmente el origen del retraso.

IMPORTANT: Angular no elimina el plugin de seguimiento de tareas de zone.js ni esta utilidad de los bundles de producción. Úsalos solo para depuración temporal de problemas de estabilidad durante el desarrollo, incluso para builds de producción optimizados.
...
## Renderización consistente entre el servidor y el cliente {#consistent-rendering-across-server-side-and-client-side}
...
## Bibliotecas de Terceros con Manipulación del DOM {#third-party-libraries-with-dom-manipulation}
...
## Scripts de Terceros con Manipulación del DOM {#third-party-scripts-with-dom-manipulation}
...
## Hidratación Incremental {#incremental-hydration}
